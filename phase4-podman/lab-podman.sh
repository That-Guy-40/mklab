...
# `podman import` — and therefore this driver's own `from-tarball` backend —
# consumes. That closes §2's loop in the return direction: everything comes
# from a chroot, so everything can go back to one.
#
# No pipe into gzip: `podman export | gzip > f` reports GZIP's exit status, and
...
cmd_export_tarball() {
    ...
    local tmp; tmp="$(mktemp "${TMPDIR:-/tmp}/lab-podman-export.XXXXXX.tar")"
    # shellcheck disable=SC2064
    trap "rm -f '$tmp'" RETURN

    log_info "podman export $cname → $out"
    podman export -o "$tmp" "$cname" || die "podman export $cname failed"
