#!/usr/bin/env bash
# test-spec-placeholders.sh — lab-podman.sh must expand @LAB_DIR@ / @REPO@ / @NETBOOT@ / @HOME@
...
source "$LAB_PODMAN" >/dev/null 2>&1 || true
...
pass "lab-podman.sh resolves @LAB_DIR@, @REPO@, @NETBOOT@ and @HOME@ inside a volumes list, leaves a spec without placeholders byte-identical, and lets no placeholder reach the container engine"
