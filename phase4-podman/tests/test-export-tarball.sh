...
#
# The plan's §9.5 table said podman could already do this — read off Appendix A's
# row *"preserve: podman → portable — `export` verb present"*.  The verb IS
# present; `lab-podman.sh export` emits kube/compose YAML, a topology SPEC.  The
...
CLONE="roundtrip-$$"
on_exit 'rm -f "$CONFIG" "$TARBALL"; cleanup_lab "$LAB"; podman rm -f "lab-$CLONE" >/dev/null 2>&1 || true; podman image rm -f "lab-from-tarball-$CLONE" >/dev/null 2>&1 || true'
...
MARKER="preserved-$$"
podman exec "$cname" sh -c "echo $MARKER > /etc/mklab-marker" \
    || fail "could not write the marker into the container"

# ── export ────────────────────────────────────────────────────────────────────
out="$("$LAB_PODMAN" export-tarball "$LAB/web" --output "$TARBALL" 2>&1)" \
...
"$LAB_PODMAN" run --name "$CLONE" --tarball "$TARBALL" --detach -- sleep 120 >/dev/null \
    || fail "from-tarball run failed — the artifact is not consumable by this driver"
got="$(podman exec "lab-$CLONE" cat /etc/mklab-marker 2>&1)" \
...
# ── refusals ──────────────────────────────────────────────────────────────────
out="$("$LAB_PODMAN" export-tarball "$LAB/nosuch" --output /tmp/never-$$.tar.gz 2>&1)" \
