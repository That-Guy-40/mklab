#!/usr/bin/env bash
# Regression (REVIEW-phase3.md §3): three container-name gates put the verdict
# DOWNSTREAM OF A PIPE — `docker ps -a --format '{{.Names}}' | grep -qx "$cname"`.
#
...
#
# It is latent rather than live: real `docker ps` writes its output in one write(2),
...
# A `docker` shim that answers `ps` with a large name list in a SINGLE write, the way
