#!/usr/bin/env bash
# TODO A.3 (docker sibling of phase4's test-start-verb.sh): `up` is create-if-absent, NOT converge — against an existing container
...
# reach around a driver to `docker start`: two owners on one lifecycle is the
# stale-record bug this repo keeps finding).
#
# The interesting assertion is not "start worked".  It is that `start` REPORTS
# FAILURE when the container does not stay running: `docker start` exiting 0 says
...
DIES="lab-${LAB}-dies"
on_exit 'rm -f "$CONFIG"; cleanup_lab "$LAB"; docker rm -f "$DIES" >/dev/null 2>&1 || true'
...
docker stop -t 1 "$cname" >/dev/null 2>&1 || fail "could not stop the fixture"
[[ "$(docker inspect -f '{{.State.Status}}' "$cname")" == exited ]] \
...
"$LAB_DOCKER" up --config "$CONFIG" >/dev/null 2>&1
[[ "$(docker inspect -f '{{.State.Status}}' "$cname")" == exited ]] \
...
out="$("$LAB_DOCKER" start "$LAB/web" 2>&1)" || fail "start failed: $out"
grep -q '^PASS: started' <<<"$out" || fail "start did not report a PASS verdict: $out"
[[ "$(docker inspect -f '{{.State.Status}}' "$cname")" == running ]] \
    || fail "start reported success but the container is not running"
note "start brought it back, and the state was read back from docker"

# ── 2. idempotent: a reconcile loop may issue it against an already-running row ─
out="$("$LAB_DOCKER" start "$LAB/web" 2>&1)" || fail "start on a running container failed: $out"
grep -q 'already running' <<<"$out" || fail "expected an already-running verdict, got: $out"

# ── 3. it does not create ──────────────────────────────────────────────────────
out="$("$LAB_DOCKER" start "$LAB/nosuch" 2>&1)" && fail "start invented a container that was never declared"
...
# A container whose command exits immediately: `docker start` returns 0 and the
...
docker create --name "$DIES" busybox:latest true >/dev/null 2>&1 \
    || fail "could not create the control container"
out="$("$LAB_DOCKER" start "$LAB/dies" 2>&1)" \
