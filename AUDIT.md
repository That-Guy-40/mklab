...
| F4 | Low | Security | ⚠️ **REOPENED 2026-08-16 for Phase 3's `export`** — see [`REVIEW-phase3.md`](REVIEW-phase3.md) P3-1. The *live* paths remain fixed: both drivers default a published port to `127.0.0.1` via `_pub_host` (`phase3-docker/lab-docker.sh:93`, `phase4-podman/lab-podman.sh:108`), with `LAB_PUBLISH_HOST` as the opt-in to a wider bind. But this row's claim that **every** publish site routes through it was scoped to the sites that hand a port to the *engine*. `lab-docker.sh export` emits the port verbatim into its compose YAML, and the documented `docker compose -f … up -d` handoff then binds `0.0.0.0` **and `[::]`** — measured end-to-end from one TOML on 2026-08-16. **Phase 4 has the same gap** — see [`REVIEW-phase4.md`](REVIEW-phase4.md) P4-5. An earlier version of this row said *"Phase 4's generated quadlets do route `PublishPort=` through `_pub_host` (`lab-podman.sh:574,612`), so the gap is Phase 3's export alone"*; that was derived from grepping `_pub_host`'s **call sites**, which is a mechanism check standing in for *"does every generated artifact carry the default?"* Phase 4 generates **two** artifacts and only the quadlet was in that list: `export --format compose` (`lab-podman.sh:1897`) emits the port verbatim. Measured 2026-08-16 from one spec — the same driver wrote `PublishPort=127.0.0.1:18097:80` into the `.container` unit and `- "18097:80"` into the compose file. *Original:* Default port publishing binds `0.0.0.0` (all interfaces) for Docker/Podman labs |
