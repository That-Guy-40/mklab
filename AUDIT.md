...
> in both. *(F4 was REOPENED 2026-08-16 when the audit found both drivers' **compose
> exports** bypassing `_pub_host`, and re-closed the same day once the generated
> artifacts were routed through it too — see the row below for what that cost.)*
> **F3** was documented and **F7** had been fixed before the list that asks for
...
| F4 | Low | Security | ✅ **RE-CLOSED 2026-08-16 — now genuinely every publish site, generated artifacts included.** This row was REOPENED earlier the same day (see [`REVIEW-phase3.md`](REVIEW-phase3.md) P3-1 and [`REVIEW-phase4.md`](REVIEW-phase4.md) P4-5) because its claim that **every** publish site routes through `_pub_host` had been scoped to the sites that hand a port to the *engine*. Both drivers' `export --format compose` emitted the port verbatim, and the documented `docker compose -f … up -d` handoff then bound `0.0.0.0` **and `[::]`** — measured end-to-end from one TOML. Fixed at `phase3-docker/lab-docker.sh:1392` and `phase4-podman/lab-podman.sh:1897`; both were watched to fail before the fix, and each carries a control asserting an **explicit** bind IP still round-trips untouched, so the default is defaulted and the opt-in is not overridden. **The lesson is about how this row got it wrong, and it is worth more than the fix:** the earlier claim was derived from grepping `_pub_host`'s **call sites**, which answers *"where is the helper called?"* when the question is *"does every artifact that publishes a port carry the default?"* — a mechanism check standing in for an outcome. Phase 4 generates **two** artifacts and only the quadlet was in that list. The durable form of this row is therefore a test per generated artifact, which is what now exists: `phase3-docker/tests/test-export-hardening.sh`, `phase3-docker/tests/test-compose-interop.sh`, `phase4-podman/tests/test-compose-export.sh`, plus the pre-existing quadlet and `run`/`up` coverage. *Original:* Default port publishing binds `0.0.0.0` (all interfaces) for Docker/Podman labs |
