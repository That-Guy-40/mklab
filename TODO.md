...
- [~] Exercise `retap` against a deliberately root-owned tap. **`retap` is PROVEN 2026-08-07** — `TUNSETIFF-FAILED errno=1` on a tap with owner uid 0 → `retap` → `TUNSETIFF-OK`, reservation byte-identical and still single. Took two failed runs, both correct: run 1 caught its OWN fixture (an owner-**less** tap is attachable by anyone; only uid 0 is G.4), run 2 caught the test asserting a message where `fabric.sh` was right. **§5 (the two mutual refusals) and §6 (Calico unchanged + teardown) are still UNKNOWN — the corrected test has not been re-run.** [Appendix P](MICRO_CLOUD_LAB_PLAN.md#appendix-p--retaps-first-privileged-run-the-test-failed-and-that-is-the-finding-2026-08-07)
