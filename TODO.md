      ...
      **New candidates the closing surfaced, none started:**
      - [x] **`here!` printed `Dictionary space overflow` and CONTINUED** (`kernel/forth.c`
            ...
            *dictionary overflow*) is thrown from C through `enterforth`, the way ppc's
            ...
            shipped `[DEFINE]` runs at dictionary-build time, where the check is off. **DECIDED
            2026-09-05: not fixed.** Nothing in the tree uses `[DEFINE]` interactively, and the proper fix
            (carving the preprocessor's buffer from the dictionary) would cost 0x2000 bytes in every
            dictionary image for a word nobody types. Recorded here so it is a decision, not a gap.
      ...
      - [x] **`readelf` does not check a duplicate `PT_INTERP`** — **second oracle wired 2026-09-04:
            ...
            is absent; CI installs `elfutils`. **Measured in CI on #403's first run** (job 101224981481, elfutils 0.190): *flags baddup.elf ('more than one INTERP entry in program header') and not good.elf; on badord.elf it says nothing about PHDR/INTERP/LOAD order*. Measured on the dev host too on 2026-09-05 once elfutils 0.190 was installed: the same three answers. **And the question "what can we decide about badord.elf" surfaced a fourth fixture** (2026-09-05): `badord.elf` moves PHDR *and* INTERP, the refusal fires on PHDR first, so `?ph-order`'s `PT_INTERP after a PT_LOAD` branch had never been exercised. `badint.elf` (`PHDR LOAD INTERP LOAD`) isolates it — and **no tool we have enforces that sentence**: readelf silent, elflint *No errors*, the kernel reads `PT_INTERP` wherever it sits. The firmware refuses it on the gABI's word alone; the track measures the oracles per run and words the verdict from the measurement. **DECIDED 2026-09-05:** (a) the stated limit is accepted as a limit — a spec clause no tool enforces is still a clause; (b) elflint's missing order check is worth telling upstream, so [`fixtures/elf-gate/UPSTREAM-elflint-no-phdr-order-check.md`](examples/openbios-the-rival-that-shipped/fixtures/elf-gate/UPSTREAM-elflint-no-phdr-order-check.md) is the report, drafted with the two three-word reproducers — filing it on sourceware's bugzilla is a human's step; (c) Acts VII and VIII of [`showcase-preboot-toolkit.sh`](examples/openbios-the-rival-that-shipped/SHOWCASE-PREBOOT-TOOLKIT.md) now drive patches 66/67 and the gate in the one-boot demo (89 s on KVM).
      History below. —
