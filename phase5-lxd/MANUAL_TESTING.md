...
services; VMs are skipped with a `# (skipped: type=vm)` comment because Compose
has no VM concept.  Each instance carries a `# dropped:` line naming exactly
which LXD-specific fields could not be represented — derived from the spec, not
a fixed list.

It needs **no engine**: `--format compose` only reads the stored spec and prints
YAML, so it works on a host with no LXD/Incus installed at all.
...
#
# WHAT THIS FILE IS NOT: a faithful copy of the lab.  Phase 5 instances are
# LXD/Incus instances, and most of what defines them (profiles, projects,
# storage pools, config keys, devices) has no compose equivalent.  The
# per-instance "dropped:" lines below name exactly what was lost, derived
# from this spec rather than from a fixed list.  `--format lxc-yaml` is the
# faithful export, and it is read from the ENGINE rather than from a spec
# file that may no longer describe reality.
services:
  "web":
    image: "images:alpine/3.23"
    container_name: "lab-demo-mixed-web"
    # dropped: config  (no compose equivalent — see --format lxc-yaml)
  "cache":
    image: "images:alpine/3.23"
