...
> **Status**: v3 (2026-07-29), ~~**decision document, nothing built yet**~~ →
> **BUILT THROUGH SLICE 5c** (status re-derived 2026-08-16). Slices **1, 2, 3, 4, 5a, 5b
...
> universal.
>
> *The struck line stood for eighteen days and seven slices.* A plan's front page is the
> first thing a reader acts on, and this one said nothing existed while `fabric.sh`,
> `lab-fc.sh`, `export-rootfs`, two chaos matrices and three example labs were all in the
> tree — the bug class this document spends [§10](#10-verification--observed-vs-merely-generated)
> and its whole defect ledger on, committed by the document.
>
...
>   and stages the kernel and rootfs inside the jail. The rootfs is **hard-linked**, not
>   copied — a copy would be a second mutable image of one guest disk, which is a drift
>   `cmd_create` already shipped once. And the rewrite is asserted rather than trusted,
>   because a Firecracker that cannot open its root device fails in a way that reads like a
>   corrupt image rather than a bad path.
>
> - **The rewrite's first version matched only the whitespace `gen_config` happens to emit**
>   (`"key": "…"`, with the space). Against a config written any other way it silently changed
>   **nothing** — matching one writer's habits rather than the format. Caught by the tool's own
>   assertion, and the test now uses the no-space form so the tolerance stays exercised.
>
> - **P7-5 arrived for the THIRD time, through a third verb.** A jailed VMM's argv carries
>   neither the host config path nor the host socket path — jailer chroots before it execs, so
>   both are in-chroot names identical for every instance. `_running_pid` now reads
>   **`/proc/<pid>/root`**, which is a *stronger* identity than an argv match and cannot be
>   spoofed by a recycled pid: that process is rooted somewhere else. The isolation boundary
>   §5.6 asks you to *read* turned out to be the thing the tool should read to know what it is
>   looking at.
>
> - **The capability is DERIVED, not proxied.** `[[ $EUID -eq 0 ]]` would be right most of the
>   time, which is the problem — it says no to a non-root process that holds CAP_SYS_ADMIN and
>   yes to a root process in a userns that does not. Bit 21 of `CapEff` is the fact. The
>   refusal fires **before** the staging copy and names both the capability and the syscall,
...
>   the filter. Asserting a difference there would be asserting something the tier does not
>   claim — and stating it is what keeps "wrapped a hypervisor in a chroot" a precise claim.
...
> ✅ **BUILT 2026-08-18 — the verb exists, and the CLI shape is not the sketch above.**
> `lab-fc.sh snapshot {create|list|restore|delete} <name> [snap]`, deliberately the same
> shape as `lab-vm.sh snapshot` so [§9.5](#95-preserve--two-tiers-and-a-derivation)'s fast
> tier can reach both phases through one form. Three findings, all of which change what
> slice 8 has to do:
>
> - **The blocker was `--no-api`, not a missing verb.** `start` ran the VMM with
>   `--no-api --config-file`, and pause / `snapshot/create` / `snapshot/load` are API-only —
>   there was nowhere to send them. `start` now passes `--api-sock` *beside* the config
>   file, so the boot is unchanged and a control channel is added. (`sun_path` is 108 bytes
>   including the NUL, and this stack has been bitten by that before, so a too-long instance
>   path falls back to a short **derived** socket name rather than failing to start.)
>
> - **A snapshot must carry the DISK, and this one does.** Firecracker's snapshot is memory
>   + device state; the guest's page cache, open inodes and dirty buffers all describe the
>   rootfs *as it was at the pause*. Restore that memory over a rootfs that has moved on and
>   **nothing errors** — you get a running guest whose kernel believes things about a
>   filesystem that are no longer true. So `snapshot create` copies the rootfs *inside the
>   pause*, making the three files one instant, and `restore` refuses a snapshot whose bytes
>   have changed **by name, with both digests**. It is expensive (a 128 MiB rootfs is
>   128 MiB per snapshot) and `create` says so — but it is also exactly what the five clones
>   below need, since each must get its own disk or they scribble over each other.
>
> - **A restored VMM has no `--config-file`**, because the snapshot carries the machine
>   configuration. That broke the P7-5 identity check (which matched only the config path in
>   argv), and would have made every restored instance invisible to `list`, `stop` and
>   `start` — P7-5 reintroduced through a new verb rather than a new bug. `_running_pid` now
>   accepts either per-instance path.
>
> **Proof it is a restore and not a reboot:**
> [`tests/test-snapshot-round-trip.sh`](phase7-firecracker/tests/test-snapshot-round-trip.sh)
> boots a real guest that prints a monotonic counter to its console, snapshots at tick *N*,
> lets the live VM run on to *N+4*, stops it, restores — and asserts the restored guest's
> **first** tick is *N+1*. A fresh boot restarts at 1; a guest that never rewound would be
> past *N+4*. The console log is append-only across restores, so it is read from a recorded
> byte offset — the same trap metal-as-a-service's stale console set. Measured: snapshot at
> 4, live reached 8, restored resumed at **5**, no kernel banner. The negative control (a
...
| gap | evidence | work |
|---|---|---|
| ~~**`phase6b-web` has zero wizards**~~ ✅ **DONE 2026-08-19** | P1 XFAIL — no wizard refs under `phase6b-web/` | port the form→preview→save flow to the web UI; `base.py`'s split makes this mostly a view layer. **It is an adapter, not a port** ([`lab_web/wizards.py`](phase6b-web/lab_web/wizards.py)): the fields are read out of the TUI wizard's own `compose_form()` by inspecting the widgets it yields, and the spec comes from its own `generate_toml()`. Nothing under `phase6b-web/` describes a wizard — a second description of a phase-7 microVM, one in Textual and one in Jinja, would drift, and the day it did one of them would generate specs the tool rejects while its own tests stayed green. Asserted as **byte-identical output through both paths** |
| ~~**no `microvm` wizard**~~ ✅ **DONE 2026-08-19** | phase 7 does not exist yet | a sixth subclass, after §5.2's schema is *derived* in slice 4. [`phase7.py`](phase6-tui/lab_tui/screens/wizards/phase7.py) — and every field is asserted against `lab-fc.sh`'s own `KNOWN_KEYS`, read out of the tool rather than copied, so a wizard cannot grow a knob the CLI lacks |
...
|---|---|---|---|
| **fast / engine-native** | qcow2 snapshot · FC snapshot+memory · LXD snapshot · `podman commit` | **running state** — but see the correction below | non-portable, version- and engine-locked |
| **portable / round-trip** | back to a tarball or chroot **+ `derivation.toml`**: source spec, artifact `sha256`s, tool versions, date | **reproducibility** | loses running state **and the image's configuration** — see below |

> ✅ **BUILT 2026-08-18 — [`examples/micro-cloud/preserve.sh`](examples/micro-cloud/preserve.sh)**,
> with [`RUNBOOK-preserve.md`](examples/micro-cloud/RUNBOOK-preserve.md) and three tests.
> `save` (both tiers) · `verify` (three outcomes) · `restore` (gated) · `capabilities`.
> The break-it row of §14 slice 7 is
> [`tests/test-preserve-gate.sh`](examples/micro-cloud/tests/test-preserve-gate.sh): a
> **one-byte** change to an artifact is refused **by name, with both digests, before
> anything is imported**, and it is asserted beside its two neighbours — an artifact nobody
> can read comes back **UNKNOWN**, not CHANGED and not a pass. It needs neither an engine
> nor root (phase 1's `export-tarball` takes a plain path), so the assertion this section
> most needs runs everywhere rather than skipping on the machines least likely to have
> podman; the live round trip through real rootless podman is its sibling.

> ⚠ **Correction to the fast-tier row, measured while wiring it up.** *"Preserves running
> state"* is true of Firecracker snapshot+memory and of a stateful LXD snapshot. It is
> **not** true of phase 2: `lab-vm.sh snapshot create` takes a `qemu-img` **internal**
> snapshot and the driver refuses to take one against a running VM (*"would corrupt a live
> disk"*), so what phase 2's fast tier preserves is **disk state at a stopped moment**.
> Still worth having, still non-portable — but a reader who took this table at its word
> would expect to resume a live guest and would not.
>
> **And the fast tier was one phase wide.** `preserve.sh --tier fast` refuses a phase whose
> driver has no engine-native verb **by name**, each refusal naming the mechanism this table
> intends *and* the verb that does not exist (`podman commit`, `docker commit`, `incus
> snapshot` stateful). Silently falling back to the portable tier would hand an operator a
> backup with no running state while they believed they had one — the failure this section
> exists to prevent, wearing a helpful face.
>
> ✅ **Firecracker's row closed 2026-08-18.** `lab-fc.sh snapshot {create|list|restore|delete}`
> now exists, so the fast tier is **two** phases wide and the one it gained is the one §5.8
> needs. The blocker was never a missing verb: `start` ran the VMM with **`--no-api`**, and
> Firecracker's pause / `snapshot/create` / `snapshot/load` are API-only, so there was
> nowhere to send them. `start` now passes `--api-sock` beside the config file — the boot is
...
> so the image `import` builds back has no `CMD`, no `ENTRYPOINT`, no `ENV`, no `WORKDIR`.
> **The filesystem survives the round trip and the *intent* does not.**
>
> ✅ **CLOSED 2026-08-20 — [TODO A.4](TODO.md).** The derivation is the right home for that
> intent and it carries it now: phases 3 and 4's `inspect` report `command`, `entrypoint`,
> `entrypoint_source`, `env` and `workdir` at `schema_version: 2`, `preserve.sh` writes the
> argv into `derivation.toml`, and `restore` prints the command that starts it — filled in
> from what was recorded rather than ending on a `<cmd>` placeholder. Measured end to end
> against live rootless podman: the restored image alone still dies at `no command or
> entrypoint provided`, and the line `restore` prints comes back `Up 1 second
> /bin/sleep 600`. The image still has no config — that is `export`, and nothing here
> changes it; what changed is that the argv now survives beside the bytes.
>
> **Three of A.4's own claims did not survive being re-derived**, which is the part worth
> keeping. *"No driver reports a container's argv"* was false — phases 3 and 4 both did,
> and both were **wrong**: docker read only `Config.Cmd`, so an `ENTRYPOINT`-only container
> was reported as having no argv at all; podman read `Cmd // Entrypoint // []`, and since
> podman flattens the entrypoint to a joined string, that field was a **string for one
> image and an array for the next**. And *"phases 3/4/5"* was two phases too many: an LXD
> instance runs the image's own init (`lab-lxd.sh` refuses a `command` key by name), and
> its `from-tarball` backend synthesises `metadata.yaml`, so Phase 5 has neither the field
> nor the defect. `lxd` stays at `schema_version: 1`; so does podman's **pod** document,
> because bumping a document that did not change is a false statement about it.
...
| `test-guided-path-is-a-view.sh` | ✅ **built 2026-08-19** — **§0.2's invariant**: every guided step's declared command resolves to a real verb of a real tool. Covers all six wizards' `run_hint()` (rendered, not read: a hint assembled at runtime would be invisible to a regex) and every `verify_cmd` in `learning-paths.toml`. The verb is **asked, not grepped** — the tool is run with the verb and with a verb nobody has, and the answers compared | that a walkthrough SUCCEEDS — `tools/wizard-walkthrough.sh` does that for the five START_HERE docs and is strictly stronger |
...
| **9** ✅ | **Two paths, finished** — **DONE 2026-08-19.** The break-it row was built FIRST, so every guided surface added after it is covered by construction — and it found a defect in itself on its first negative control (see the gate column) | ~~a `microvm` wizard~~ ✅ · ~~the learning path~~ ✅ (`micro-cloud` is no longer `coverage_exempt`; one step per build slice — the count is `tools/paths.py --check`'s to keep, not this sentence's, and it read *nine* here for a day after the path had eleven) · ~~web wizards (§8.2 gap)~~ ✅ — an **adapter** over the TUI wizards, not a second implementation: the web form is rendered from `compose_form()`'s own widgets and the spec from `generate_toml()`, asserted byte-identical through both paths. `save` writes one `.toml` inside the repo and returns the raw-path commands; it executes nothing | a beginner reaches a booted microVM guided-only; you reach one raw-only | ~~`test-guided-path-is-a-view.sh` bites when a guided step does something the CLI cannot~~ ✅ — and the probe it uses was **wrong in the dangerous direction**: `lab-vm.sh boot` is not a verb, but that tool's help says "--secure-boot", and normalising the verb out of only ONE side of the comparison reported it PRESENT. Caught by running the control rather than reasoning about it; the same one-sided form was latent in `test-preserve-capability-table.sh` and is fixed there too, with a control for the shape |
| **10** ✅ | **The demo** — **DONE 2026-08-20.** `micro-cloud.sh up` brought five instances of four kinds up on one fabric and `down` took them away leaving Calico untouched; §9.3's isolation matrix reached **5 of 5 rows measured, 0 UNKNOWN** — the microVM row answered by the lab's **own** `api1` over vsock, not a stand-in. Catalog routing landed 2026-08-19 ([the path](examples/learning-paths/path-micro-cloud.md)). All seven rows of [`MANUAL_TESTING.md`](examples/micro-cloud/MANUAL_TESTING.md) are ✅ | `micro-cloud.sh up`, five instances, §15's transcript; catalog routing | the transcript reproduces; §9.3's isolation matrix | teardown leaves **nothing of ours** and **everything of Calico's** |
...
> ✅ **BUILT 2026-08-19 · OBSERVED 2026-08-19/20 — the transcript below is a TARGET, and the
> run that answered it is recorded elsewhere.** All seven rows of
> [`MANUAL_TESTING.md`](examples/micro-cloud/MANUAL_TESTING.md)'s success signature are **✅**:
> rows 1–3 and 6–7 on 2026-08-19 (fabric up with Calico unmoved, five instances of four kinds,
> name resolution across the L2 at 0% loss, teardown leaving nothing of ours and everything of
> theirs), and rows 4–5 on 2026-08-20 (each microVM's own identity from one image, MMDS half
> included; §9.3's matrix at **5 of 5 measured, 0 UNKNOWN**).
>
> **This block said `every row still ⬜` until 2026-08-20**, having been written the day the
> lab was *built* and never revisited on the days it was *run*. It is the record-outlives-its-
> subject shape, aimed at the one claim a reader of a plan most needs to be true: whether the
> capstone has been measured. Nothing failed — the sentence was merely false, in the document
> that decides whether anyone bothers to run it.
>
> Two lines of the transcript below were called out as wrong before the run, and the run
> confirmed both:
>
> * **the addresses are not promised.** The transcript says `api1 10.71.0.11`, `edge …101`,
>   `db …102`; what the fabric actually handed out, by arrival order inside
>   `MC_DHCP_LO..MC_DHCP_HI`, was **`api1 .101`, `api2 .102`, `edge .103`, `db .104`**. What is
>   stable is each instance's **MAC**, derived from its name — which is why row 4 can assert
>   identity at all.
...
>   shows all five. `edge`'s own first-boot ping did lose that race, exactly as predicted.
...
3. ~~**Decision G — MAAS registry reuse** (§8.4). Recommendation: invoke for deploy drivers,
   separate registry initially, revisit at slice 6.~~ → **SETTLED 2026-08-16**
   ([§8.4a](#84a-decision-g--settled-2026-08-16-derive-the-facts-record-only-the-intent)):
   **no registry of facts.** Enumerating MAAS's fourteen registry fields split them cleanly —
   everything that is a *fact about the instance* is derivable from an engine, and everything
   that is not derivable is *intent or history*. `apply` reads reality on every pass, which
   deletes MAAS's "ground the registry in reality" phase rather than reimplementing it.
   Option 1 still stands for the deploy drivers. *This entry kept saying "revisit at slice 6"
   for four days after slice 6 revisited it — and [§18.6](#186-order-of-work) item 9 struck
   the same decision through on the day it was made. Two records of one decision, one of them
   stale, is the shape §8.4a itself argues against.*
...
| 9 | ~~**slice 6 — the control plane**~~ → **DONE 2026-08-18** (opened 2026-08-16) | decision E is settled, so the seam it needs is no longer an open question | ~~an `fc` backend~~ ✅ 2026-08-16 ([`phase6-tui/lab_tui/backends/fc.py`](phase6-tui/lab_tui/backends/fc.py), 13 tests; `phase6b-web` inherited it with no second implementation) · ~~and topology slot~~ ✅ 2026-08-16 ([`topology.py`](phase6-tui/lab_tui/topology.py) — `create --config` + `start <name>`, since Phase 7 has no `up`) · ~~decision **G**~~ ✅ 2026-08-16 ([§8.4a](#84a-decision-g--settled-2026-08-16-derive-the-facts-record-only-the-intent) — no registry of facts; the enumeration split cleanly into derivable facts and un-derivable intent) · ~~**`apply` a no-op on pass two**~~ ✅ 2026-08-16 ([`apply.py`](phase6-tui/lab_tui/apply.py) — it acts on 2 of the diff's 6 kinds; the no-op holds because pass two's DIFF finds nothing, not because a flag was remembered, and it is verified against **live rootless podman in CI**, not only against fakes) · ~~**the break-it row**~~ ✅ 2026-08-18 — a graded matrix over **6 layers**, not one scenario ([`phase6-tui/lab_tui/chaos.py`](phase6-tui/lab_tui/chaos.py) + [`tests/test_control_plane_chaos.py`](phase6-tui/tests/test_control_plane_chaos.py)). **It found a LIED on its first run**: `reconcile` trusted each backend's own lab filter, so a backend whose filter was absent let another lab's `web` answer this lab's question and `apply` reported CONVERGED over a lab with nothing running. Fixed by re-checking the scope we asked for. **Slice 6 is complete** |
| 10 | ~~**slice 7** — preserve~~ → **DONE 2026-08-18** | the backups asked for in §0, and the first slice whose gap was four phases wide rather than one | ~~both tiers + `derivation.toml`~~ ✅ · ~~`lab-vm.sh export`~~ ✅ (#233, **rootless** through a libguestfs appliance) · the portable tier's lost image configuration became **TODO A.4**, named rather than glossed |
| 11 | ~~**slice 8** — the fleet~~ → **DONE 2026-08-19** | clones need a preserved artifact to clone *from*, so it cannot precede 10 | ~~snapshot/restore~~ ✅ (#236) · ~~five warm clones from one memory image~~ ✅ **2.1 s for five** against ~0.5 s to boot one · ~~the jailer tier~~ ✅ on its **sixth** privileged run · the entropy hazard reproduced only after the row **rewrote itself** — the prescribed demonstration measures a window a few reads wide |
| 12 | ~~**slice 9** — two paths, finished~~ → **DONE 2026-08-19** | the guided surfaces are a *view* of the raw ones, so they can only be finished once there is something to view | ~~a `microvm` wizard~~ ✅ · ~~the learning path~~ ✅ · ~~web wizards as an adapter, not a second implementation~~ ✅ · the break-it row was built **first** and caught a defect in itself on its first negative control |
| 13 | ~~**slice 10** — the demo~~ → **DONE 2026-08-20** | everything above it, at once, on real hardware | ~~five instances of four kinds up and down on one fabric, Calico unmoved~~ ✅ · ~~§9.3's isolation matrix~~ ✅ **5 of 5 measured, 0 UNKNOWN** · all seven [`MANUAL_TESTING.md`](examples/micro-cloud/MANUAL_TESTING.md) rows ✅. **This table stopped at item 9 until 2026-08-20** — four slices landed underneath an order-of-work list that never learned they had, while item 9's own gate column already ended on *"Slice 6 is complete"*. A row whose status cell and gate cell disagree is the cheapest possible instance of this plan's own bug class |
...
> ✅ **BOTH ROWS ARE NOW CLOSED — DHCP exhaustion 2026-08-06, the tap-address scenario
> 2026-08-07.** Added as a dated pointer rather than an edit: the bullets below are the
> record of what was UNKNOWN on 2026-08-02 and stay as written (§17.4 Q6's rule — *the
> appendix is the record and is immutable; the instrument is code and must be true when it
> runs*). The first bullet's *"deferred to a host without a live cluster"* was answered by
> building one: a disposable microk8s in an unprivileged `lab-vm.sh` guest,
> [`examples/nested-calico-sandbox/`](examples/nested-calico-sandbox/), where a genuine
> `fabric.sh` tap given an address on purpose captured the guest cluster's tunnel —
> **F.6 reproduced deliberately** instead of survived once
> ([Appendix Q](#appendix-q--the-sandbox-packaged-g9-closed-on-the-real-artifact-2026-08-07)).
