...
| **9** ◐ | **Two paths, finished** — **the guard, the sixth wizard and the learning path are DONE 2026-08-19; the web wizards remain.** The break-it row was built FIRST, so every guided surface added after it is covered by construction — and it found a defect in itself on its first negative control (see the gate column) | ~~a `microvm` wizard~~ ✅ · ~~the learning path~~ ✅ (`micro-cloud` is no longer `coverage_exempt`; nine steps, one host-safe machine-checked checkpoint) · **web wizards (§8.2 gap)** ← remains | a beginner reaches a booted microVM guided-only; you reach one raw-only | ~~`test-guided-path-is-a-view.sh` bites when a guided step does something the CLI cannot~~ ✅ — and the probe it uses was **wrong in the dangerous direction**: `lab-vm.sh boot` is not a verb, but that tool's help says "--secure-boot", and normalising the verb out of only ONE side of the comparison reported it PRESENT. Caught by running the control rather than reasoning about it; the same one-sided form was latent in `test-preserve-capability-table.sh` and is fixed there too, with a control for the shape |
