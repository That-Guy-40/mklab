...
> and every precursor §18.6 listed has landed. **Next is slice 6, the control plane** — see
> §14. Decision **E** was answered by slice 5a ([§8.3](#83-the-unsolved-half--one-seam-or-four)
> shape **(b)**), which is the question slice 6 depends on.
>
...
| 9 | **slice 6 — the control plane** ← **NEXT** (2026-08-16) | decision E is settled, so the seam it needs is no longer an open question | an `fc` backend and topology slot; `apply` a no-op on pass two |
