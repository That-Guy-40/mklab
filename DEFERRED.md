...
|---|---|---|---|
| D2 | **1a** — the `cbfs*`, `event-*` and `sha256` tracks are outside Tier B's `DEFAULT_TRACKS`, so the CBFS and attestation halves are verified only on the dev host | [Item 1a/1b](TODO.md#item-1a1b--two-hygiene-gaps-the-same-audit-surfaced-small-do-alongside-or-right-after) | Opus 4.8, alongside or right after D1 |
