...
| D3 | **1b** — ✅ **DONE 2026-09-20 (`fix/ci-qemu-install-strict`).** `ci.yml`'s "Install qemu-system" step now **hard-asserts** `command -v qemu-system-x86_64` after the install (`::error::` + `exit 1`) instead of the swallowed `\|\| echo "::warning::"` — a 2 s apt that installed nothing now fails the step LOUDLY rather than turning nine phase-2 QEMU-gated rows into a quiet sweep of SKIPs inside a job that claimed to install the tool. The narrower aarch64/xorrisofs targets stay named warnings (fewer rows). **Verified live:** on the PR's first run the assert correctly went red when an apt-mirror `404` left qemu-system-x86_64 uninstalled — the gate working, not a code fault. | [Item 1a/1b](TODO.md#item-1a1b--two-hygiene-gaps-the-same-audit-surfaced-small-do-alongside-or-right-after) | Opus 4.8, alongside or right after D1 |
| D4 | **2** — ✅ **DONE 2026-09-21 (`feat/tier0-conformance-contract`).** `dsl/CONTRACT.md` at `contract v1`; the refusal convention is unified at the seam every module already passes through — `struct.fth`'s `chk` gained a `chk-catch` flag so `validate ( xt -- c-addr u )` runs the *same* checks under `catch` and returns a **named reason** instead of aborting (the abort+`want=/got=` path is byte-for-byte unchanged, so `elf-methods` is untouched). All four divergent modules conform through **sidecars** (`dsl/{elf,cbfs,fdt,evlog}-conform.fth`, base readers left standalone-loadable) with a biting negative control each; **the real defect is fixed in `dsl/cbfs.fth`** — `cbfs-list` refused a corrupt first entry by name where it used to print `CBFS-END` and look empty (failing→passing pair proven: pre-fix the checker's REGRESSION guard fires). `dsl/contract.fth` carries the registry (`register-module`/`.modules`/`need-module` → `MODULE X: not loaded`). `tests/test-contract-conformance.sh` (headless, in `run-all.sh`) grades all four and **proves itself first** on 1 conformant + 3 planted-defect fixtures, and asserts the slim-profile separability guarantee. The two consumers (UKI workbench, openbios-modern-filesystems) carry an honest `contract v1` pin. | [Item 2](TODO.md#item-2--the-tier-0-conformance-contract--slim-profile-checker-the-roadmaps-unbuilt-piece) | Opus 4.8, **after** D1 |
