...
| [`run-e2e-measured.sh`](run-e2e-measured.sh) + [`lib/tpm_xml.py`](lib/tpm_xml.py) | the IMAGE+MEASURED live path (author-run): TPM+UEFI on the domain, a quote delivered to the sink, a policy captured from that boot, and a deliberately bent policy that must be REFUSED |
