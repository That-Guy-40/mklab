...
| File | What it is |
|---|---|
| [`image/flake.nix`](image/flake.nix) + [`configuration.nix`](image/configuration.nix) (shared lab identity — serial console, autologin — in [`lab-common.nix`](image/lab-common.nix)) | The NixOS image (pins `nixos-unstable` → **systemd 261**). Grows per spike. |
