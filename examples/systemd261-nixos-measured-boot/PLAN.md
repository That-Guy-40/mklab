...
| F | `ConditionFraction=` 3-VM mock fleet | deterministic staged rollout across machine-IDs | ✅ **DONE** — `fleet.toml` (3 VMs, distinct machine-ids) + `fleet-demo.sh`; sweeping `ConditionFraction=N%` shows monotonic canary rollout (0→0→1→2→2 as 10→90%), no orchestrator. Syntax = **percentage** (`99%`, not `0.99`); it **is** in nixpkgs' systemd 261 |
| G | TPM2-sealed LUKS + attestation stub | `systemd-cryptenroll` PCR policy; PCR quote | ✅ **DONE** — on the measured VM, `systemd-cryptenroll --tpm2-pcrs=7+11` seals a LUKS keyslot; `systemd-cryptsetup` unseals with the TPM alone (no passphrase); an AK-signed `tpm2_quote` over PCR 7+11 + a nonce verifies (`tpm2_checkquote`); extending PCR 11 makes the TPM **refuse** to unseal (seal bound to measured state). Sealed golden image (`sealed.nix`) builds + boots measured (PCR 11 populated) + its baked `/etc/lab/sealed-luks-demo` PASSes; ships over the **same Tier-B iPXE/UEFI path** (`--sealed`). HONEST: swtpm ("IBM") is plumbing, not a trust anchor; the AK has no manufacturer EK cert |
