...
case "$FLAVOR" in
  ...
  x86)
    ...
    if [[ "$MEDIA" == disk ]]; then
        ...
        MEDIA_ARGS=(-cdrom "$ISO"); DEV='/ide@1/cdrom@0'; MEDIUM="an ISO9660 CD"
    fi

    # x86 has no `boot cd:` shortcut for a client: load it, then go. The BACKSLASH
    # in the arg is load-bearing — grubfs converts \ to /, a forward slash gets
    # eaten by the device-path parser (POC-7).
    # shellcheck disable=SC2016  # `$load` is the Forth word, not a shell var
    LOAD='" '"$DEV"':\x5c'"$PROG"'" $load\r'
    case "$PROG" in
      hello)
    ...
    LOG="$WORKDIR/smoke-client-x86-$PROG-$MEDIA.log"; rm -f "$LOG"
    note "booting revived OpenBIOS-x86 + our $PROG on $MEDIUM, driving \$load $DEV + go → $LOG"
    ...
    python3 "$REPO/tools/drive-pty-repl.py" "$LOG" --timeout "$TMO" --echo-gate \
        ...
        -- qemu-system-x86_64 -M "pc,accel=$ACCEL" -m 512 \
           ...
           "${MEDIA_ARGS[@]}" -display none -serial mon:stdio -no-reboot
    RC=$?
    if grep -aq "memtest: FAIL" "$LOG"; then
    ...
    pass "revived OpenBIOS-x86 loaded our C client '$PROG' from $MEDIUM and it $WHAT over the IEEE 1275 client interface" ;;
