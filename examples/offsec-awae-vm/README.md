...
The **full `offsec-awae` build is now verified end-to-end too**: the
`offsec-awae 2025.4.0` chroot images into a BIOS/extlinux qcow2 that boots to the
Kali serial `login:` with `root`/`toor` working (same kernel `6.19.14+kali`,
generic initramfs over virtio). One wrinkle worth knowing: a chroot built
*before* the `90-lab-vm.conf` dracut fix carries a host-only initramfs and must
be regenerated by hand before imaging — see `INITRAMFS-TROUBLESHOOTING.md`. A
from-scratch build bakes the generic initramfs in via the TOML, so this only
affects pre-fix chroots. See `MANUAL_TESTING.md`.
