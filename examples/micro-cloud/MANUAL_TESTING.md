...
| 2 | five instances of four kinds are up | `micro-cloud.sh status` shows `api1`, `api2` running, `edge` running, `db` and `metrics` listed by their own drivers | ⚠️ **4 of 5** — `api1`, `api2`, `edge`, `metrics`. `db` blocked on a spec key phase 5 does not have (LEDGER L10-9); fixed, not yet re-run |
...
| 5 | §9.3's matrix has five rows, not two | `test-isolation-matrix.sh` prints no UNKNOWN rows | ⚠️ **3 of 5 reachable** — host, `metrics`, `edge` measured. `db` needs the L10-9 fix re-run; **`api1`/`api2` are structurally unreachable** from outside and will stay UNKNOWN until they boot slice 5c's vsock-agent rootfs |
