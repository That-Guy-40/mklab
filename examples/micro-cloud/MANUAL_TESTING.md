...
| 4 | each microVM has its **own identity from one image** | `lab-fc.sh inspect api1` and `api2` show different MACs and addresses from the same rootfs lineage; MMDS answers `instance-id` inside each guest | ⚠️ **half observed 2026-08-19** — both booted from copies of one source image and took *different* MACs and *different* reserved leases (`.101`/`.102`), each derived from its own name. The **MMDS half is still unchecked here**: nothing in these runs read `instance-id` at `169.254.169.254` from inside a guest, and slice 2 proving it once is not this lab proving it now |
| 5 | §9.3's matrix has five rows, not two | `test-isolation-matrix.sh` prints no UNKNOWN rows | ⚠️ **4 of 5 measured 2026-08-19**, and **the fifth is no longer structural (2026-08-20)** — host, `metrics`, **`db`** (7 processes, PID 1 systemd, its own boot_id and netns) and `edge` were measured on the privileged run; the **microVM row is now measured unprivileged**, by booting slice 5c's vsock-agent image and running the *same* probe commands over vsock (59 processes, PID 1 `init`, **its own boot_id**, `dmesg` readable because it is its own kernel). Its guest is booted by the test, as the container row's is |
