...
| 5 | §9.3's matrix has five rows, not two | `test-isolation-matrix.sh` prints no UNKNOWN rows | ⚠️ **3 of 5 measured** — host, `metrics`, `edge`. `db` was Running but the probe's own regex missed `Running` (LEDGER L10-13), fixed and not yet re-run. **`api1`/`api2` are structurally unmeasurable** and will stay UNKNOWN until they boot slice 5c's vsock-agent rootfs |
