...
| 5 | §9.3's matrix has five rows, not two | `test-isolation-matrix.sh` prints no UNKNOWN rows | ⚠️ **4 of 5 measured 2026-08-19** — host, `metrics`, **`db`** (7 processes, PID 1 systemd, its own boot_id and netns) and `edge`. **`api1`/`api2` are structurally unmeasurable** from outside and stay UNKNOWN until they boot slice 5c's vsock-agent rootfs — the only row of §9.3 whose absence is not a to-do |
