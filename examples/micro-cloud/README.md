...
| **`tests/test-retap-recovers-a-root-owned-tap.sh`** | 3 (break pass) | ⏳ **RUN 2026-08-07 — and it FAILED, correctly, on its own fixture.** `retap` is the verb added for the root-owned-tap defect (G.4) and had never been called by anything. §3 staged the break with a bare `ip tuntap add` (no `user`) — and uid 1000 attached to that tap **perfectly well**, so the test refused to proceed rather than "prove" a repair of something never broken. An owner-**less** tap and a root-**owned** one look identical in `ip link show` and behave in opposite directions: with neither owner nor group set, `tun_not_capable()` lets **anyone** attach ([P.1](../../MICRO_CLOUD_LAB_PLAN.md#p1-the-first-privileged-run-failed-on-its-own-fixture-and-that-is-the-result)). Staging corrected to `user root` (owner uid 0) and the fixture is now checked before the ioctl is asked. **`retap`'s own verdict is still UNKNOWN — the corrected test has not been re-run.** Needs root; SKIPs without it |
