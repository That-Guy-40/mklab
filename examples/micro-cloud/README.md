...
| **`tests/test-retap-recovers-a-root-owned-tap.sh`** | 3 (break pass) | ✅ **`retap` PROVEN 2026-08-07** — after two failed runs, both of which failed *correctly*. Run 1 died on its own fixture: §3 staged the break with a bare `ip tuntap add` and uid 1000 attached fine, because an owner-**less** tap is attachable by **anyone** (`tun_not_capable()`); only owner uid **0** is G.4. Run 2, staging `user root`, executed the real thing: `TUNSETIFF-FAILED errno=1 (Operation not permitted)` on the broken tap → `retap` → `TUNSETIFF-OK`, with the DHCP reservation **byte-identical and still single** and the tap addressless on `br-mc0`. It then failed §5 against a **correct** `fabric.sh`: `tap` has two guards and the test asserted the second one's message while standing in the first one's state. Corrected to stage both states and assert the outcome (`grep -c` still 1) rather than the sentence. **§5 and §6 are still UNKNOWN — the corrected test has not been re-run.** [Appendix P](../../MICRO_CLOUD_LAB_PLAN.md#appendix-p--retaps-first-privileged-run-the-test-failed-and-that-is-the-finding-2026-08-07) |
...
> ⏳ **One command is owed, and it is short.** `retap`'s test exists but has never
> executed an assertion. From the repo root:
>
> ```bash
> sudo bash examples/micro-cloud/tests/test-retap-recovers-a-root-owned-tap.sh
> ```
>
> It refuses to touch a fabric it did not create, tears down from its EXIT trap, and
> keeps its log on failure. Until it runs, every "retap works" statement in this repo is
> a reading of the source, not a measurement.
