...
| P3 scripts | [`build-st.sh`](build-st.sh) (build stmgr+stboot, assemble the stboot UKI) · [`make-ospkg.sh`](make-ospkg.sh) (wrap+sign the installer as an OSPKG; `--rogue` = neg) · [`run-stboot.sh`](run-stboot.sh) (OVMF verify / `--negative`) || pxeboot scripts | [`fetch-go.sh`](fetch-go.sh) · [`coreboot-qemu-q35-pxeboot.config`](coreboot-qemu-q35-pxeboot.config) · [`serve-netboot.sh`](serve-netboot.sh) · [`fetch-netboot-os.sh`](fetch-netboot-os.sh) · [`run-coreboot-pxe.sh`](run-coreboot-pxe.sh) — the P1 scaffolding (see POC-PXEBOOT.md) |
