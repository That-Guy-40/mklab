#!/usr/bin/env bash
# reset-demo-rocky.sh — HANDS-OFF proof of the Rocky/RHEL rd.break root-password
# reset on a real, kickstart-installed Rocky Linux 9.  The Rocky analogue of
# reset-demo.sh (which does the Kali init=/bin/bash reset).
#
# Calls setup-rocky-target.sh to build + pre-stage the target (unless one already
# exists), then serial-drives the CIQ rd.break method and verifies it:
...
# (append one word — the CIQ method) rather than the command line: Rocky's grub2
# redraws aggressively over serial and drops input on long typed lines, so the
# driver runs at a slower --char-delay and we append the minimum.
#
# Usage:   ./reset-demo-rocky.sh            # setup if needed, then reset + verify
#          SKIP_SETUP=1 ./reset-demo-rocky.sh   # reuse an already-prepared target
...
NEW_PW="${NEW_PW:-toor}"
CHAR_DELAY="${CHAR_DELAY:-0.08}"   # slower than the 0.04 default: Rocky grub2 drops input under redraw

log() { printf '\n\033[1;36m[demo-rocky]\033[0m %s\n' "$*" >&2; }
die() { printf '\n\033[1;31m[demo-rocky] FAIL:\033[0m %s\n' "$*" >&2; exit 1; }
...
if [ "${SKIP_SETUP:-0}" != 1 ] && { [ ! -f "$BOOTINFO" ] || ! phase2-qemu-vm/lab-vm.sh inspect "$VM" >/dev/null 2>&1; }; then
    log "no prepared target found — running setup-rocky-target.sh first (~10-15 min)"
    "$HERE/setup-rocky-target.sh"
fi
[ -f "$BOOTINFO" ] || die "no $BOOTINFO — run setup-rocky-target.sh (or unset SKIP_SETUP)"
...
# Each step is EXPECT-confirmed live; rc==0 ⟺ every marker appeared.  The GRUB
...
STEPS
    printf '\n\033[1;32m[demo-rocky] PASS\033[0m — the Rocky rd.break reset is verified end-to-end on real Rocky Linux 9.\n' >&2
