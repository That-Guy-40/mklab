#!/usr/bin/env bash
# setup-almalinux-target.sh — build a REAL AlmaLinux 9 install and pre-stage it as a
# root-password-reset target, then stop it so you can HAND-WALK the rd.break reset.
#
# The AlmaLinux analogue of setup-rocky-target.sh — same RHEL-family method, just a
# different distro.  The RHEL-family reset is rd.break (break into the dracut
...
# examples/almalinux-kickstart-gallery/ (variant: gencloud), which is already
# serial-ready (the kickstart bakes console=ttyS0) and sets the "forgotten" root
# password directly (the gallery's --root-pw).  AlmaLinux's gencloud kickstart bakes
# `bootloader --timeout=0` (the GRUB menu is HIDDEN), so the only pre-stage is
# widening that to an interruptible 5 seconds with a visible menu.
#
#   stage (installer + kickstarts --root-pw + select gencloud) -> nginx :8181
#   -> Anaconda install (unattended, ~10-15 min) -> log in root/$FORGOTTEN_PW ->
#      set GRUB_TIMEOUT=5 + _STYLE=menu + grub2-mkconfig -> capture kver/root dev -> stop.
#
# After it finishes, HAND-WALK the reset:  RUNBOOK-rd-break.md
#   phase2-qemu-vm/lab-vm.sh console almalinux-kickstart-install
# or run the hands-off proof:  ./reset-demo-almalinux.sh
#
...
GALLERY_TOML="examples/almalinux-kickstart-gallery/almalinux-kickstart-gallery.toml"
VARIANT="gencloud"
VM="almalinux-kickstart-install"
KS="almalinux-kickstart/almalinux-9.${VARIANT}-x86_64.ks"
...
log()  { printf '\n\033[1;36m[setup-alma]\033[0m %s\n' "$*" >&2; }
die()  { printf '\n\033[1;31m[setup-alma] ERROR:\033[0m %s\n' "$*" >&2; exit 1; }
...
# ── 1. stage the AlmaLinux installer + kickstart catalog, select gencloud ───────
log "1/6 staging the AlmaLinux installer + kickstarts (root pw = $FORGOTTEN_PW), selecting $VARIANT"
examples/almalinux-pxe-lab/fetch-almalinux-installer.sh --release 9 --arch x86_64 >/dev/null
examples/almalinux-kickstart-gallery/fetch-kickstarts.sh --root-pw "$FORGOTTEN_PW" >/dev/null
examples/almalinux-kickstart-gallery/select-kickstart.sh "$VARIANT" >/dev/null
...
for u in vmlinuz initrd.img images/install.img "$KS"; do
...
log "3/6 installing AlmaLinux via Anaconda (unattended; this takes ~10-15 min)"
...
# gencloud is serial-ready (console=ttyS0), so the post-install getty's "login:"
...
until grep -qa 'login:' "$INSTALL_LOG" 2>/dev/null; do
    ...
    if grep -qaiE 'Pane is dead|installation failed|Traceback|dracut: FATAL|kernel panic|Modules.Storage has failed' "$INSTALL_LOG"; then
...
log "    install finished — AlmaLinux is on disk and booting"

# ── 5. pre-stage: widen the hidden (timeout=0) GRUB menu to an interruptible 5s,
#       capture boot info.  AlmaLinux logs in as root directly (the kickstart set the
#       forgotten pw); no sudo, no console baking.  Retried up to 3x without re-install.
log "5/6 pre-staging: GRUB_TIMEOUT 0 -> 5 + STYLE=menu (grub2-mkconfig); capturing kver/root dev"
KVER=""; ROOTDEV=""; ok=0
...
cat >&2 <<EOF
  ...
  …or run the hands-off proof:
    examples/root-password-reset/reset-demo-almalinux.sh   # serial-drives rd.break + verifies
