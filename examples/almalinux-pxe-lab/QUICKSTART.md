# QUICKSTART — AlmaLinux 9 zero-touch PXE install

Boot a blank VM over the (virtual) network and have it install **AlmaLinux 9**
with no keystrokes:

```
SeaBIOS → NIC's iPXE ROM → TFTP boot.ipxe (run directly) → HTTP(vmlinuz+initrd+stage2) → Anaconda → kickstart → reboot into AlmaLinux
```

Copy-paste from the **repo root**. For the full design write-up see
[`README.md`](README.md); for the deep debugging log see
[`MANUAL_TESTING.md`](MANUAL_TESTING.md). The Rocky twin lab is documented in
[`../rocky-pxe-lab/QUICKSTART.md`](../rocky-pxe-lab/QUICKSTART.md), and the combined
Rocky+Alma runbook is [`../PXE-INSTALL-QUICKSTART.md`](../PXE-INSTALL-QUICKSTART.md).
...
> **`$HOME` note.** `almalinux-pxe-lab.toml` hardcodes `/home/sqs/netboot` in two
> places (the nginx `volumes =` line and `pxe_dir =`). TOML does no shell expansion,
> so if your `$HOME` is not `/home/sqs`, edit the TOML and replace `/home/sqs` with
> your home path. The fetch script already writes to `~/netboot` automatically.
...
# 2. Render the per-MAC kickstart (the VM's NIC is pinned to 52:54:00:a1:9a:01).
#    gen-almalinux-ks.sh stays in netboot/ (it's the generic copier Rocky shares);
#    with no --template it defaults to this lab's almalinux-zerotouch.ks.
...
# 5. Create + start the installer VM (unattended; ~5–25 min depending on mirror).
...
phase2-qemu-vm/lab-vm.sh console almalinux-pxe-install   # watch the install; Ctrl-] detaches
```
...
5. **Second boot:** `vda` is now bootable (bootindex 0) → boots AlmaLinux; the NIC
   ROM is never reached again. **Done.**

---

## Notes — why it is built this way

These fixes are baked into `almalinux-pxe-lab.toml`; you don't need to do anything,
...
- **Resumable `install.img` download** — `fetch-almalinux-installer.sh` uses
  `curl -fSL -C - --retry 8 --retry-all-errors`, because the public CDNs routinely
  drop that ~1.2 GB transfer partway. Re-running the fetch resumes it. It verifies
  vmlinuz/initrd.img/install.img against the tree's `.treeinfo` (AlmaLinux's
  per-dir `CHECKSUM` 404s and never covered install.img).
...
instead of QEMU's slirp. A vendor PXE ROM that isn't iPXE must chainload the
`ipxe.pxe` binary, so set `pxe_bootfile = "ipxe.pxe"` for that case.
...
> **Verification status:** verified directly on KVM — the NIC's native iPXE ran
> `boot.ipxe` and fetched `vmlinuz` + `initrd.img` + the **full 1.26 GB**
> `install.img` stage2 + the kickstart (nginx confirmed; no 814 MB cutoff at 4 GB
> RAM), then Anaconda installed and rebooted into **AlmaLinux 9.8 (Olive Jaguar)**
> (logged in `lab`/`lab`; `/dev/vda` partitioned with the `almalinux-root`/`-swap`
> LVM, booting from disk — loop terminated).
