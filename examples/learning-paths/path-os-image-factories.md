<!-- GENERATED by tools/paths.py from examples/learning-paths.toml — do not edit by hand; edit the TOML and run `tools/paths.py render`. -->

# OS image factories: two ways to bake a bootable image 🟡

**Goal.** Bake a whole bootable OS image two fundamentally different ways. First *assemble the rootfs*: a declarative debos recipe (the Debian image builder), then Kali's kali-vm wrapper built on the very same engine. Then *drive the installer*: Kali's retired Packer scripts boot the real installer ISO and script it with a preseed — the opposite mechanism, and the bridge back to the preseed labs.

**Audience:** you've turned a rootfs into a bootable VM by hand — now do it with a factory  
**Effort:** ⏱ half-day+  
**Prerequisites:** [Chroot → running system: the cross-phase bridges](path-chroot-to-system.md)

| # | Lab | You'll learn | Checkpoint (observable) |
|---|---|---|---|
| 1 | [`debian-vm-builder`](../debian-vm-builder/) | debos directly: a YAML recipe (debootstrap → apt → image-partition → filesystem-deploy → systemd-boot) assembled inside a fakemachine KVM build VM — no installer, no by-hand loop-mounting. | `build-debian-vm.sh` bakes a trixie qcow2; `run-graphical.sh` boots it under OVMF to a `debian-vm login:` (SSH `debian`/`debian`). |
| 2 | [`kali-vm-builder`](../kali-vm-builder/) 🔒 | Kali's official `kali-vm` is a thin wrapper around the SAME debos engine, producing the image you'd otherwise download — now the wrapper reads clearly, because you saw the machinery underneath. | `build-kali-vm.sh --headless` builds a QCOW2; `run-graphical.sh` boots it to a Kali login. |
| 3 | [`kali-packer-vagrant`](../kali-packer-vagrant/) 🔒 | The OPPOSITE mechanism: Kali's retired Packer scripts don't assemble a rootfs — they boot the real installer ISO, type a `boot_command` over VNC that points d-i at a preseed served over HTTP, run the unattended install, then SSH in to provision and package a Vagrant `.box`. Packer automates the human install; debos skips it. This is also the bridge back to the preseed labs — `http/preseed.cfg` is just a d-i preseed, delivered differently. | `build-kali-box.sh --validate-only` → `validate: OK`; a full `build-kali-box.sh` bakes `packer_kalirolling_libvirt_amd64.box`; `run-graphical.sh` unpacks the QCOW2 and boots it to a Kali `vagrant` login. |
| 4 | [`almalinux-packer-images`](../almalinux-packer-images/) | The same drive-the-installer mechanism, on the OTHER installer: AlmaLinux's official factory boots a netinstall ISO and points ANACONDA at a kickstart served over HTTP, where Kali's points debian-installer at a preseed. One `boot_command` line is the whole difference — read the two side by side. And the provenance argument flips: this upstream is ALIVE, so the vendored copy is a dated snapshot rather than a rescue, verified on every use so it cannot quietly become something else. | `fetch-cloud-images.sh` prints `archive verified: 563 files match` and stages a tree with no `.git`; `tests/test-offline-archive.sh` → `PASS`.<br>🔎 auto-checkable → marker `563 files: on disk == in SHA256SUMS == tracked by git` 🖥️ |

### Optional side-quests

- [`nix-build-box`](../nix-build-box/) — a third substrate — Nix. A reusable, pinned, flakes-enabled `nix` in a rootless container; the systemd-261 NixOS image factory bakes its dm-verity/UKI image *inside* this box. Verified end-to-end here (`nix build .#hello` → `Hello, world!`).
- [`nixos-ipxe-deploy`](../nixos-ipxe-deploy/) — the *deploy* half — a reusable block that lays a built NixOS onto a machine's disk over iPXE two ways: a netboot installer that `nixos-install`s a target (BIOS), or a deployer that `dd`s a whole-disk image (UEFI). Importable modules future labs parameterize; applied in `systemd261-nixos-measured-boot/`.

---

← back to the [learning-paths hub](README.md) · reference catalog: [`00-INDEX.md`](../00-INDEX.md)
