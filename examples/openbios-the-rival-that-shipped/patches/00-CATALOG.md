...
| kind | count | |
|---|---|---|
| `UPSTREAM-BUG` | 30 | defects any user of `openbios/openbios` would hit. The candidate set, if the decision above is ever revisited |
...
| scope | count | |
|---|---|---|
| shared | 38 | touches a path some *other* architecture's build also compiles — this is where a pin bump conflicts |
