...
| [`showcase-rescue.sh`](showcase-rescue.sh) | UKI Spike 11, the rescue capstone (**in progress**): deliberately break a boot and rescue it with no USB/chroot, the negative control as the point — the un-edited artifact fails first. The **initrd** and **config** acts are built: a no-initrd boot panics (`VFS: Unable to mount root fs`) rescued by the good initrd on the boot line; and a real busybox initramfs whose init reads a broken `/etc/rescue.conf` hangs, the firmware fixes it in place with `dsl/cpio-edit.fth`, and the rescued image boots. The cmdline act (Spike 6, OVMF) remains |
