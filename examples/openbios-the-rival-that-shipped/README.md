...
| [`patches/13-amd64-loader-forth.patch`](patches/13-amd64-loader-forth.patch) | compiles the Forth loader into amd64 (x86 parity) — a **prerequisite that is inert today**: `load`+`go` cannot reach `(init-program)` on *either* arch, and the patch header records the probe that shows it |
