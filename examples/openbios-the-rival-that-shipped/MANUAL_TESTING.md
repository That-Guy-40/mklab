...
| **`ob_pci_unmap` was the same leak, one caller in** (named by the audit, fixed 2026-09-03) | a BAR a config callback mapped at *probe* (`sungem_config_cb`, BAR0) was "unmapped" with `ofmem_unmap()` — translation back, both claims kept — so `10 bar-map` on the sungem answered **`ffffffff`, every time** | patch 61: `ob_pci_unmap` = `ofmem_release()`, and `pci-map-out` calls it | ppc row with `-device sungem`: `SGM=80200000`, beside `MAPY=ffffffff` (nothing released → still conflicts) |
| **an ihandle of 0 read as an instance** (the audit's "`$call-parent` from a parentless instance GPFs") | `s" x" 0 $call-method` fetched `0 >in.device-node @` — amd64's real-mode IVT, `rax=f000ff54f000ff7b`, **general protection fault**; x86 walked to `-21` by luck | patch 62: `$call-method` and `$call-parent` `abort"` by name, the shape `?my-self` already had | x86/amd64/ppc: `NIRC=-2` + `$call-method: ihandle is 0`; a one-level bridge instance: `NPRC=-2` + `no parent instance.`; no exception line in the log |
