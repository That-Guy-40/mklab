#!/usr/bin/env bash
# test-smoke-persist-os-flash.sh — run smoke-openbios.sh's `persist-os-flash` track under this suite.
#
...
exec "$(dirname -- "${BASH_SOURCE[0]}")/../smoke-openbios.sh" persist-os-flash
