#!/usr/bin/env bash
# test-smoke-file-writer.sh — run smoke-openbios.sh's `file-writer` track under this suite.
#
...
# It `exec`s: the verdict line, the SKIP when openbios-unix is not built, and the
# exit code are all the driver's, which is what keeps one place to type a track by
# hand and one place for it to be wrong.
exec "$(dirname -- "${BASH_SOURCE[0]}")/../smoke-openbios.sh" file-writer
