#!/usr/bin/env bash
# test-smoke-memory-available.sh — run smoke-openbios.sh's `memory-available`
# track under this suite.
#
...
exec "$(dirname -- "${BASH_SOURCE[0]}")/../smoke-openbios.sh" memory-available
