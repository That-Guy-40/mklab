#!/usr/bin/env bash
# Shared helpers for openbios-the-rival-that-shipped's tests.
#
# THE NET IS COPIED, NOT REWRITTEN. Everything above the lab-specific section is
# phase7-firecracker/tests/lib.sh verbatim — the exemplar CLAUDE.md names — because a
# second implementation of a safety net is a second thing to get wrong, and this one has
# tools/check-harness-net.sh's seven sections pointed at it. tests/test-harness-net.sh
# runs that checker inside this suite, so the copy is held to the same standard rather
# than trusted for being a copy.
#
# Same skip/fail/pass/note contract as every other suite in the repo.
set -euo pipefail

TEST_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
readonly TEST_DIR
# shellcheck disable=SC2034  # LAB_DIR and REPO are this library's public surface: tests
# that source it use them to reach the drivers and tools/. shellcheck analyses lib.sh on
# its own, where nothing consumes them.
readonly LAB_DIR="${TEST_DIR}/.."
...
# Belt-and-suspenders: a `die` inside a driver is an exit, and an unguarded one would
