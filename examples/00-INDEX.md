...
| [`linux-proc-vfs-internals/`](linux-proc-vfs-internals/README.md) | 🔬 A **C + `/proc`** box for Ciro S. Costa's four-part **[ops.tips](linux-proc-vfs-internals/upstream-tutorial/README.md)** `/proc` series (vendored byte-exact), in **two container sets** on **Debian 13** *and* **Alpine**. **Set A** (unlimited) — *What is /proc?* + *How is /proc able to list PIDs?*: a `learner` + `~/proc-lab/` sandbox with `open-fd.c` and a **raw-`getdents64`** `list-pids.c` + `demo.sh`; spotlights procfs as a **VFS** (size-0 files generated on read; PID-namespaced listings). **Set B** (512 MiB-capped, swap off) — *Why top/free show wrong memory* + *Resource limits under the hood*: `mem-hog.c` (**cgroup-v2 OOM-kill** at the cap) + `limit-open-files.c` (**`prlimit()`** get-and-set, `EPERM` on hard-raise), with **lxcfs** rewriting `/proc/meminfo` to the cap. Divergences: Alpine needs `linux-headers`; glibc `openat` vs musl/BusyBox `open`. Verified end-to-end, all four base×set combos. |
