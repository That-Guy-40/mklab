...
| Dir | What you get |
|---|---|
| [`floppinux/`](tiny-linux-experiments/floppinux/) | Krzysztof Jankowski's **FLOPPINUX** — a whole Linux on a 1.44 MB floppy — operationalized for Debian. A standalone, **rootless** `build-floppinux.sh` cross-builds kernel 6.14.11 + static BusyBox 1.36.1, packs an XZ initramfs (device nodes via `fakeroot`), and writes a `syslinux` FAT floppy you boot with `qemu-system-i386 -fda -cpu 486`. The "physical floppy + syslinux" cousin of the in-RAM micro-linux track; no `lab-vm.sh` TOML (it has no `-fda` backend). ⚠️ throwaway — boots to a passwordless root shell, no networking. |
