...
| [`linux-proc-vfs-internals/`](linux-proc-vfs-internals/README.md) | 🔬 A **C + `/proc`** box for Ciro S. Costa's two **[ops.tips](linux-proc-vfs-internals/upstream-tutorial/README.md)** articles — **"What is /proc?"** and **"How is /proc able to list PIDs?"** (vendored byte-exact) — a C toolchain + `strace` + a `learner` user + a `~/proc-lab/` sandbox holding the articles' `open-fd.c` and a **raw-`getdents64`** `list-pids.c` + a runnable `demo.sh`, on **Debian 13** *and* **Alpine**. Spotlights procfs as a **VFS** (size-0 files generated on read; PID-namespaced listings) with an **lxcfs**-serves-`/proc/meminfo` gem, and the toolchain divergences (Alpine needs `linux-headers`; GNU `ls` uses `openat`, BusyBox `open`). Verified end-to-end on both. |
