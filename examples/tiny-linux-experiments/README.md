...
| [`toybox-mkroot/`](toybox-mkroot/) | Rob Landley's **toybox** (the 0BSD multicall binary that ships in Android) + its built-in **mkroot** distro builder (`make root`) operationalized — a rootless `build-toybox-mkroot.sh` that builds the multicall binary, then bakes a **kernel + toybox initramfs + `run-qemu.sh`** into a bootable system. The **toybox** cousin of `floppinux`/`micro-linux` (both **BusyBox**): here the builder ships *with* the userspace. Full-from-source (host gcc, no cross toolchain) **and** a `--prebuilt <arch>` fast lane for ~22 CPU families. Verified end-to-end. Includes [`LINEAGE-aboriginal.md`](toybox-mkroot/LINEAGE-aboriginal.md) — Landley's **Aboriginal Linux** ("simplest Linux that compiles itself") distilled. See its [README](toybox-mkroot/README.md). |
