...
| **Disk format** | `--disk-format {qcow2\|raw}` (default `qcow2`). `raw` attaches a **per-instance copy** of `--image` rather than a CoW overlay — the semantics `lab-fc.sh` uses, so a QEMU guest and a Firecracker microVM can boot the *same* image with the same storage stack. The declared format is checked against the file before the VM is created, because QEMU otherwise catches a mismatch only at `start` |
